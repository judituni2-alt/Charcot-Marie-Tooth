...
# --------------- Ejecución en segundo plano (opción -b) -----------------
...
# inmediato. Así, aunque se cierre la terminal el pipeline seguirá corriendo
# en el servidor.
#
...
#
# La variable de entorno BACKGROUND evita que el proceso relanzado
...
bam_pasa_criterios() {
    local qc_dir="$1"
    local pct
    pct=$(grep "mapped (" "$qc_dir/qc_align/${SAMPLE_ID}_flagstat.txt" | head -1 \
          | grep -oP '\(\K[0-9.]+(?=%)')
...
round_bam=0
while ! bam_pasa_criterios "$QC_DIR"; do
