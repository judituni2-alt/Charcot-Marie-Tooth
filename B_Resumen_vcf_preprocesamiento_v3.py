...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resumen de un VCF (nº variantes, tipo, por cromosoma). Sin QUAL, sin CSV."
    )
    parser.add_argument(
        "--vcf",
        required=True,
...
def summarize(vcf_path: Path) -> None:
    ...
    logger.info("Resumen calculado en %.2f s", time.perf_counter() - t0)
